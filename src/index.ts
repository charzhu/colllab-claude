  ...
  try {
    switch (name) {
      ...
      case "collab_scan_project": {
        const { include_file_samples, max_files } = args as {
          include_file_samples?: boolean;
          max_files?: number;
        };
...
Call collab_init with your recommended policies array.`,
      ...
      case "collab_init": {
        const { policies, default_trust } = args as {
          policies?: Array<{
            pattern: string;
            trust: TrustLevel;
            reason: string;
        ...
        if (policies && policies.length > 0) {
          const result = await initializeCollabWithPolicies({
            policies: policies as TrustPolicy[],
            default_trust,
          });
