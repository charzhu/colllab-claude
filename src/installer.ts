...
async function installMcpServer(): Promise<void> {
  let config: McpConfig = (await readJson<McpConfig>(MCP_FILE)) || {};
...
async function uninstallMcpServer(): Promise<void> {
  const config = await readJson<McpConfig>(MCP_FILE);
...
async function installHooks(): Promise<void> {
  ...
  let settings: Settings = (await readJson<Settings>(SETTINGS_FILE)) || {};
  ...
  const hasPreEdit = preToolUse.some((h) =>
    h.hooks?.some((hook) => hook.command.includes("collab"))
  );
  if (!hasPreEdit) {
    preToolUse.push({
      matcher: "Edit|Write",
  ...
  const hasPostEdit = postToolUse.some((h) =>
    h.hooks?.some((hook) => hook.command.includes("collab"))
  );
  if (!hasPostEdit) {
    postToolUse.push({
...
async function uninstallHooks(): Promise<void> {
  ...
  if (settings.hooks.PreToolUse) {
    settings.hooks.PreToolUse = settings.hooks.PreToolUse.filter(
      (h) => !h.hooks?.some((hook) => hook.command.includes("collab"))
    );
  }
